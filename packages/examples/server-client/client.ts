...
async function main() {
  ...
  new ZATCAInvoice({ props: props1, acceptWarning: true }); // Validate XML locally
  clientInvoices.push({
  ...
  new ZATCAInvoice({ props: props2, acceptWarning: true }); // Validate XML locally
  clientInvoices.push({
  ...
  clientInvoices.forEach((inv) => {
    console.log(
      `  ${inv.invoiceSerialNumber}: ${inv.props.previousInvoiceHash.substring(0, 15)}... → ${inv.serverResponse?.invoiceHash?.substring(0, 15) || "pending"}...`,
    );
  });
