...
function parseTLVTags(
  bytes: Buffer
): { tag: number; length: number; value: Buffer }[] {
  const tags: { tag: number; length: number; value: Buffer }[] = [];
  let offset = 0;
  while (offset + 2 <= bytes.length) {
    const tag = bytes[offset];
    const length = bytes[offset + 1];
    const value = bytes.slice(offset + 2, offset + 2 + length);
    tags.push({ length, tag, value });
    offset += 2 + length;
  }
  return tags;
}
...
function makeQRProps(vatName?: string): ZATCAInvoiceProps {
  return {
    ...
    egsInfo: {
      ...
      model: "IOS",
      name: "QR-TEST",
      vatName: vatName ?? "QR Test Company",
...
describe("tLV structure verification", () => {
  ...
  it("tLV tag 4 = tax-inclusive total (~115 for price=100 + 15% VAT)", () => {
    ...
    expect(Number.parseFloat(decoded)).toBeCloseTo(115, 1);
  });

  it("tLV tag 5 contains a non-empty VAT total value", () => {
    const invoice = buildInvoice(makeQRProps());
    const qr = generatePhaseOneQR(invoice);
    const bytes = Buffer.from(qr, "base64");
    const tags = parseTLVTags(bytes);
    ...
    expect(Number.parseFloat(decoded)).toBeGreaterThan(0);
    expect(Number.parseFloat(decoded)).toBeCloseTo(15, 1);
  });
});
...
describe("unicode edge cases", () => {
  ...
  it("seller name with numbers and hyphens — encodes as plain ASCII bytes", () => {
    ...
    expect(tags[0]).toHaveLength(numericName.length);
  });
});
