...
export class XMLDocument {
  ...
  constructor(xml_str?: string) {
    ...
    if (xml_str) {
      ...
      this.xml_object = {
        "?xml": { "@_encoding": "utf8", "@_version": "1.0" },
      };
    }
  }
  ...
  private getElement(
    ...
    if (path_query === "" || !xml_object) {
      return { last_tag, parent_xml_object, xml_object };
    }
  ...
  get(path_query?: string, condition?: any): XMLQueryResult {
    ...
    if (query_result && !Array.isArray(query_result)) {
  ...
  delete(path_query?: string, condition?: any): boolean {
    ...
    if (_.isEmpty(query_result)) {
      return false;
    }

    if (Array.isArray(parent_xml_object[last_tag])) {
      parent_xml_object[last_tag] = _.filter(
        parent_xml_object[last_tag],
        (element) => !_.matches(condition)(element)
      );
  ...
  set(
    ...
    try {
      if (Array.isArray(parent_xml_object[last_tag][tag])) {
