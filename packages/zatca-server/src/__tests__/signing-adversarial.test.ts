...
describe("signing adversarial coverage", () => {
  describe(cleanUpCertificateString, () => {
    ...
    it("extracts known fixture body exactly", () => {
      expect(cleanUpCertificateString(SAMPLE_CERT_PEM)).toBe(
        SAMPLE_ZATCA_TEST_CERT_BODY
      );
    });
  });
  ...
  describe(getCertificateInfo, () => {
    it("returns hash, issuer, serial number, public key, and signature", () => {
      ...
      expectTypeOf(info.issuer).toBeString();
      expectTypeOf(info.serial_number).toBeString();
      expect(Buffer.isBuffer(info.public_key)).toBeTruthy();
      expect(Buffer.isBuffer(info.signature)).toBeTruthy();
    });
  ...
  describe(createInvoiceDigitalSignature, () => {
    ...
    it("throws for invalid private key", () => {
      expect(() =>
        createInvoiceDigitalSignature("aGFzaA==", "not-a-key")
      ).toThrow();
    });
  ...
  describe(generateSignedXMLString, () => {
    it("returns signed invoice XML, invoice hash, and QR", () => {
      ...
      expectTypeOf(result.signed_invoice_string).toBeString();
      expectTypeOf(result.invoice_hash).toBeString();
      expectTypeOf(result.qr).toBeString();
    });
